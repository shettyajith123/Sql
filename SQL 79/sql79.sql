...
(
select *, 
row_number() over(partition by traveler order by date) as first_city_rn,
row_number() over(partition by traveler order by date desc) as last_city_rn
from travel_history
), cte2 as (
Select traveler,max(case when first_city_rn = 1 then start_city end) as first_city,
max(case when last_city_rn = 1 then end_city end) as end_city
from cte
group by traveler
)
Select count(*) as cnt from cte2 where first_city = end_city